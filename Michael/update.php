...
if (isset($_POST["submit"])) {
    ...
    if ($pictureArray->error === 0) {
        ...
        $sql = "UPDATE user SET f_name = '$f_name', s_name = '$l_name', email = '$email', date_of_birth = '$date_of_birth', u_picture = '$pictureArray->fileName' WHERE u_id = {$id}";
    } else {
        $sql = "UPDATE user SET f_name = '$f_name', s_name = '$l_name', email = '$email', date_of_birth = '$date_of_birth' WHERE u_id = {$id}";
    }
    if (mysqli_query($connect, $sql) === true) {
        $class = "alert alert-success";
        $message = "The record was successfully updated";
        $uploadError = ($pictureArray->error != 0) ? $pictureArray->ErrorMessage : '';
        header("refresh:3;url=update.php?id={$id}"); //save id because after post value is gone!
    } else {
...
<body>
    <div class="container">
        ...
        <h2>Update</h2>
        <img class='img-thumbnail rounded-circle' src='pictures/<?php echo $data['u_picture'] ?>' alt="<?php echo $f_name ?>">
        <form method="post" enctype="multipart/form-data">
            <table class="table">
                ...
                <tr>
                    ...
                    <!-- page is refreshed after clicking submit hence the id must be stored! -->
